...
public class PostsAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private List<PostsVo.Posts> data;
    ...
    public PostsAdapter(Context context, List<PostsVo.Posts> data) {
    ...
    public List<PostsVo.Posts> getAll() {
        return data;
    }
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == TYPE_ITEM) {
            View view = inflater.inflate(R.layout.posts_item_view, parent, false);
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, final int position) {
        if (holder instanceof ItemViewHolder) {
            ...
            final PostsVo.Posts detail = data.get(position);
            ((ItemViewHolder) holder).nickname.setText(detail.getNickname());
            imageLoader.displayImage(Constants.imageUrl +
                    detail.getUserinfo().getIcno(), ((ItemViewHolder) holder).iv_image, options);
            ((ItemViewHolder) holder).content.setText(detail.getContent());
            if (detail.getClassfy() == 1)
                ((ItemViewHolder) holder).classfy.setText("美食");
            if (detail.getClassfy() == 2)
                ((ItemViewHolder) holder).classfy.setText("情感");
            if (detail.getClassfy() == 3)
                ((ItemViewHolder) holder).classfy.setText("工作");
            if (detail.getClassfy() == 4)
                ((ItemViewHolder) holder).classfy.setText("生活");
            if (detail.getClassfy() == 5)
                ((ItemViewHolder) holder).classfy.setText("拼车");
            ((ItemViewHolder) holder).addtime.setText(detail.getAddtime());
    ...
    static class ItemViewHolder extends RecyclerView.ViewHolder {
        ...
        TextView nickname, content, classfy, addtime;
        ImageView iv_image;
        ...
        public ItemViewHolder(View view) {
            ...
            classfy = (TextView) view.findViewById(R.id.classfy);
            addtime = (TextView) view.findViewById(R.id.addtime);
